...
exports.createTenant = async (req, res) => {
  try {
    ...
    // Basic Validation
    if (!name || !id || !email || !subdomain) {
      return res.status(400).json({ message: 'Name, ID, Email, and Subdomain are required.' });
    }

    // Check for duplicates (Email, ID, or Subdomain)
    const existingTenant = await Tenant.findOne({
      $or: [{ email }, { id }, { subdomain }]
    });
    ...
    const newTenant = new Tenant({
      name,
      id, // Assuming ID is passed manually or generated by FE/Counter Service
      email,
      subdomain
    });
